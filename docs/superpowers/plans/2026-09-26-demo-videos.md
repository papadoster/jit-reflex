...
## Журнал изменений (до полного прогона)

- **2026-09-26, ревью кода** (`4c8b36c`), правило выбора не затронуто:
  - после таймаута (256 шагов при 260 кадрах) пауза в 1 с обрезалась, теперь последний кадр держится;
  - в заголовках A и C дописано «retrained locally»: модели мира маковские, а числа B1 получены с моделями с пода;
  - добавлены тесты на согласованность пресетов и на заморозку панели;
  - перенос подписи 0.5 вместо 0.6.
- **Smoke** (`demo c --seeds 2`, 43 с) — подпись влезает. GIF для A пишем шириной 960, а не 720: у трёх панелей при 720 картинки слишком мелкие, а файл на 2 панели занял всего 0.37 МБ.
- **Видео E4 командой из README больше не воспроизводится побайтно.** Рефакторинг `kick` тут ни при чём: старый `video.py` на текущем коде даёт тот же файл. Коммиты точного ускорения B1 (`71c78b1`, `090d016`) меняют округление, и эпизод seed 0 расходится с 25-го шага. `results/video_kick.mp4` воспроизводится на `f9be043`; в README это будет сказано.
- **После прогона A:** счёт по 16 seed'ам — RTC 5, pred 12, reflex 11; pred заметно выше, чем в B1 (42%). Проверка тем же `eval_flow` на swimmer, seed 10 из B1, 256 эпизодов, маковская модель мира: pred 57.4%, reflex 76.2%. В B1 с моделью с пода было 43.0% и 70.7% (`results/demo/wm_check_swimmer.csv`). Значит, на swimmer маковская модель усиливает pred. Моделей с пода нет: архив с пода собирался без `*.pkl`. Поэтому, по решению владельца, в подпись A добавлена строка об этом. Правило выбора эпизода и выбранный seed не меняются, A перегоняется только ради подписи.
- **Строка «Goal:» в подписях.** Зелёные фигуры Kinetix при рендере 128 px не видны: во всех трёх видео ноль зелёных пикселей. Цель взята из ролей в JSON уровня (1 — зелёная, 2 — синяя):
  - swimmer: переднее звено пловца должно коснуться синего столба;
  - catapult: сбить перекладину с двух стоек на синий пол.

  Видео перегоняются ради подписи. Правило и seed'ы те же.
